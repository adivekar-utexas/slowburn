...
class TestEstimateCostMicrodollars:
    ...
    def test_unknown_model_raises(self) -> None:
        ...
        with pytest.raises(ModelNotFoundError):
            PricingCache.estimate_cost_microdollars("fake-model-999", 1000, 500)
...
class TestActualCostMicrodollars:
    ...
    def test_unknown_model_tier3_raises(self) -> None:
        ...
        with patch("slowburn.pricing.litellm.completion_cost", side_effect=Exception("nope")):
            with pytest.raises(ModelNotFoundError):
                PricingCache.actual_cost_microdollars(response, model="fake-model-999")
    ...
    def test_all_tiers_fail_raises(self) -> None:
        ...
        with patch("slowburn.pricing.litellm.completion_cost", side_effect=Exception("nope")):
            with pytest.raises(ModelNotFoundError):
                PricingCache.actual_cost_microdollars(response, model=None)
    ...
    def test_tier1_success_bypasses_model_check(self) -> None:
        ...
        cost = PricingCache.actual_cost_microdollars(response, model="some-unknown-thing")
        assert cost == 10_000
