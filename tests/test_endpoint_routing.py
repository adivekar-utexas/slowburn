...
class TestMultiEndpointRouting:
    ...
    def test_endpointconfig_instance_rejected(self) -> None:
        ...
        cfg = EndpointConfig(
            ...
            timeout=10.0, max_rpm=100, max_input_tpm=1000, max_output_tpm=1000,
            max_concurrent_calls=10, budget_usd=5.0, window="daily",
            rate_limit_algorithm="GCRA",
        )
        # @validate rejects at the pydantic layer with ValidationError; our
        # explicit TypeError raise inside create_llm is the fallback path.
...
class TestSharedLimitObjects:
    ...
    def test_unset_endpoints_share_one_call_limit_instance(self) -> None:
        """All endpoints inheriting max_rpm share the same CallLimit object."""
        llm = create_llm(
            model="default",
            max_rpm=300,
            endpoints=[
        ...
        try:
            ...
            # And the shared CallLimit has the global capacity.
            assert call_limits[0].capacity == 300
        finally:
            llm.stop()

    def test_overriding_endpoint_gets_private_call_limit(self) -> None:
        """An endpoint that sets max_rpm gets its own CallLimit; the rest share."""
        llm = create_llm(
            model="default",
            max_rpm=300,
            endpoints=[
                {"model": "m-A", "endpoint_id": "A"},  # inherits 300
                {"model": "m-B", "endpoint_id": "B", "max_rpm": 1000},  # override
    ...
    def test_unset_endpoints_share_one_resource_limit(self) -> None:
        """max_concurrent_calls inheritance shares a single ResourceLimit."""
        llm = create_llm(
            model="default",
        ...
        try:
            ...
            for ls in llm.limits.limit_sets:
                for lim in ls.limits:
                    if getattr(lim, "key", None) == "concurrent_calls":
    ...
    def test_overriding_endpoint_gets_private_resource_limit(self) -> None:
        """An endpoint that sets max_concurrent_calls gets its own ResourceLimit."""
        llm = create_llm(
            model="default",
            max_concurrent_calls=50,
            endpoints=[
                {"model": "m-A", "endpoint_id": "A"},
                {"model": "m-B", "endpoint_id": "B", "max_concurrent_calls": 200},
            ],
        )
        try:
            res_limits = []
